...
export class JwtAdapter {
    ...
    private verifyToken(token: string, algorithm: string) {
        return new Promise((resolve) => {
            jwt.verify(token, this.secret, { algorithms: [algorithm], issuer: this.issuer }, (err, decoded) => {
                if (!err) {
                    resolve(decoded);
                } else {
                    throw err;
    ...
    verifyAccessToken(token: string) {
        return this.verifyToken(token, this.algorithms.HS256);
    }
    verifyRefreshToken(token: string) {
        return this.verifyToken(token, this.algorithms.HS512);
    }
